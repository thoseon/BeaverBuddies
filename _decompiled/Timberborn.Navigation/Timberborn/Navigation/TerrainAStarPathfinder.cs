...
internal class TerrainAStarPathfinder : ILoadableSingleton
{
	...
	public void FillFlowFieldWithPath(TerrainNavMeshGraph terrainNavMeshGraph, PathFlowField flowField, int startNodeId, int destinationNodeId)
	{
		...
		if (_flowField.CheckedPath(startNodeId, destinationNodeId) || !HeuristicallyReachable(terrainNavMeshGraph, startNodeId, destinationNodeId))
		{
			return;
		}
	...
	private static bool HeuristicallyReachable(TerrainNavMeshGraph terrainNavMeshGraph, int startNodeId, int destinationNodeId)
	{
		if (terrainNavMeshGraph.IsOnNavMesh(startNodeId))
		{
			return terrainNavMeshGraph.IsOnNavMesh(destinationNodeId);
		}
		return false;
	}

	private void VisitNeighbors(TerrainNavMeshGraph terrainNavMeshGraph, AStarNode node)
	{
