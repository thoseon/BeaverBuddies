using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Linq;
using System.Reflection;
using Bindito.Core;

namespace Timberborn.SingletonSystem;

internal class SingletonListener : IInjectionListener, IProvisionListener
{
	private readonly List<object> _providedSingletons = new List<object>();

	private readonly List<object> _injectedSingletons = new List<object>();

	private bool _collected;

	private ImmutableArray<object> _allSingletons;

	private readonly Dictionary<Type, bool> _typeCache = new Dictionary<Type, bool>();

	public IEnumerable<object> Collect()
	{
		if (!_collected)
		{
			_allSingletons = _providedSingletons.Concat(_injectedSingletons).Distinct().ToImmutableArray();
			_collected = true;
		}
		return _allSingletons;
	}

	void IProvisionListener.Listen(object injectee)
	{
		if (ObjectIsSingleton(injectee))
		{
			ThrowIfCollected(injectee);
			_providedSingletons.Add(injectee);
		}
	}

	void IInjectionListener.Listen(object injectee)
	{
		if (ObjectIsSingleton(injectee))
		{
			ThrowIfCollected(injectee);
			_injectedSingletons.Add(injectee);
		}
	}

	private bool ObjectIsSingleton(object target)
	{
		Type type = target.GetType();
		if (_typeCache.TryGetValue(type, out var value))
		{
			return value;
		}
		bool flag = TypeIsSingleton(type);
		_typeCache.Add(type, flag);
		return flag;
	}

	private static bool TypeIsSingleton(Type type)
	{
		if (type.GetCustomAttribute(typeof(SingletonAttribute), inherit: true) != null)
		{
			return true;
		}
		Type[] interfaces = type.GetInterfaces();
		for (int i = 0; i < interfaces.Length; i++)
		{
			if (interfaces[i].GetCustomAttribute(typeof(SingletonAttribute), inherit: true) != null)
			{
				return true;
			}
		}
		return false;
	}

	private void ThrowIfCollected(object injectee)
	{
		if (_collected)
		{
			throw new InvalidOperationException($"Attempting to add an instance of {injectee.GetType()} after collecting.");
		}
	}
}
