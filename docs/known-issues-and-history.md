...
| (working tree, 2026-09-16) | Fix pathfinding desync after a construction changes the nav mesh | Timberborn applies "instant" nav mesh and district changes in `NavigationSynchronizer.LateUpdateSingleton` (per frame) while regular ones go through `Tick()`. `Walker.PathIsTooFarFromDistrict` (`IsOnInstantDistrictRoadSpill`), `Citizen.UnassignDistrictIfCutOff` (`GlobalReachabilityService`), `ReachableConstructionSite` and the district assigners read the instant copy, so whether a bucket saw a change made earlier in the same tick depended on where a frame boundary fell. Trace (error report 2026-09-16, tick 401): `finished pathfinding; reachable = False` on the client vs `True` on the host for the same beaver; the host stranded it one tick later. | `Fixes/InstantNavMeshFix.cs`: a service wrapping `NavigationSynchronizer`, called from a prefix on `TickableEntityBucket.TickAll` so every bucket starts with the instant changes applied; bound in `Plugin.cs`. Traces `Applying instant navmesh changes` and `Citizen … unassigned from district` in `DesyncPatches.cs`. Runtime confirmation still outstanding. |
