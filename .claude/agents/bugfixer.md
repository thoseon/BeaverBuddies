...
6. **Non-determinism?** Check `DeterminismService.ShouldFreezeSeed`'s decision order, the RNG blacklist, `Doc/ClassesWithRandom.txt` (legend `~`/`+` = suspects), `Doc/Movement.md` for movement, `Fixes/WaterSourceFix.cs` for water. Grep the decompiled class on the stack for `Time.deltaTime`; the mod patches `Time.time` only.
